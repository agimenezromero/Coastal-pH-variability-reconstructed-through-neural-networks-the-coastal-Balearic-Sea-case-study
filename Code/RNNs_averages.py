...
def RNN(Nf=100, tol=0.8):
    ...
    while N < Nf:
        ...
        if final_train_loss_RNN < tol:
            ...
            X_TREND = time_years[y_pred_noscale_RNN[:, 0].astype('str') != 'nan'].reshape(-1,1)
            ...
            xdata = X_TREND.flatten()
            ydata = Y_TREND.flatten()

            for T in np.linspace(0.95, 1.05, 1000):

                w = np.pi*2/ T

                def func_2(x, A, slope, C, D):

                    return A*np.sin(x*w + C) + slope*x + D

                popt, pcov = curve_fit(func_2, xdata, ydata)

                RMSE = np.sqrt(np.mean(np.power(ydata - func_2(xdata,*popt), 2)))

                errors.append(RMSE)
                Ts.append(T)

                slopes_2.append(popt[1])

            slope_curve_fit = slopes_2[np.argmin(errors)]
            ...
            slopes_curvefit.append(slope_curve_fit)

    return slopes, intercepts, train_errors, val_errors, N_epochs, training_time, slopes_curvefit
...
def LSTM_NN(Nf=100, tol=0.8):
    ...
    while N < Nf:
        ...
        if final_train_loss_LSTM < tol:
            ...
            X_TREND = time_years[y_pred_noscale_LSTM[:, 0].astype('str') != 'nan'].reshape(-1,1)
...
def BI_LSTM_NN(Nf=100, tol=0.8):
    ...
    while N < Nf:
        ...
        if final_train_loss_BI_LSTM < tol:
            ...
            X_TREND = time_years[y_pred_noscale_BI_LSTM[:, 0].astype('str') != 'nan'].reshape(-1,1)
...
def BI_GRU_NN(Nf=100, tol=0.8):
    ...
    while N < Nf:
        ...
        if final_train_loss_BI_GRU < tol:
            ...
            X_TREND = time_years[y_pred_noscale_BI_GRU[:, 0].astype('str') != 'nan'].reshape(-1,1)
...
if type_ == "RNN":
    ...
    slopes, intercepts, train_errors, val_errors, N_epochs, training_time, slopes_cf = RNN(Nf)
    
elif type_ == "LSTM":
    
    slopes, intercepts, train_errors, val_errors, N_epochs, training_time, slopes_cf = LSTM_NN(Nf)
    
elif type_ == "BI_LSTM":
    
    slopes, intercepts, train_errors, val_errors, N_epochs, training_time, slopes_cf = BI_LSTM_NN(Nf)
    
elif type_ == "BI_GRU":
    
    slopes, intercepts, train_errors, val_errors, N_epochs, training_time, slopes_cf = BI_GRU_NN(Nf)
...
header = "Slope Intercept Train_error Val_error N_epochs Training_time Slopes_cf"

np.savetxt("Results_%s.txt" % type_, 
           np.transpose([slopes, intercepts, train_errors, val_errors, N_epochs, training_time, slopes_cf]),
           header=header)
